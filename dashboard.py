...
FIXES & ADDITIONS:
1. Added 'Vehicle Color' column with dynamic UI color dots.
2. Added 'Extended Crop' column to show the full vehicle image alongside the Plate crop.
3. Updated the manual entry form to support the new vehicle_color field.
...
def color_chip(color):
    """Creates a UI pill with a colored dot representing the vehicle color."""
    if isinstance(color, str) and color.strip() and color.lower() != "unknown":
        color_map = {
            "black": "#222222", "white": "#F8F9FA", "silver/gray": "#ADB5BD",
            "red": "#FA5252", "green": "#40C057", "blue": "#339AF0",
            "yellow": "#FCC419", "orange": "#FD7E14", "brown": "#8D6E63"
        }
        c_val = color.lower().strip()
        hex_c = color_map.get(c_val, "#7C8B9E")
        
        # Draw a small colored dot next to the text
        dot = f'<span style="display:inline-block; width:8px; height:8px; border-radius:50%; background-color:{hex_c}; margin-right:6px; border:1px solid #444;"></span>'
        return f'<span class="chip chip-photo" style="display:flex; align-items:center; width:max-content;">{dot}{html.escape(c_val.upper())}</span>'
    return '<span class="chip chip-none">—</span>'
...
def render_detection_table(df: pd.DataFrame):
    """Renders the data table including the new Color and Extended Crop columns."""
    rows_html = []
    for _, row in df.iterrows():
        ts = html.escape(str(row.get("timestamp", "—")))
        det_id = html.escape(str(row.get("id", "—")))
        ...
        # Dynamically build local static URLs for both crops based on the plate string
        plate_url = f"/static/crops/{plate}_plate.jpg" if plate else None
        vehicle_url = row.get("evidence_url") # The API primarily stores the extended crop now
        
        # Fallback if evidence_url is missing but we know the plate
        if not vehicle_url and plate:
            vehicle_url = f"/static/crops/{plate}_vehicle.jpg"
...
<td class="mono">{cam_id}</td>
<td>{vehicle_chip(row.get("vehicle_type"))}</td>
<td>{color_chip(row.get("vehicle_color"))}</td>
<td>{license_plate_chip(plate)}</td>
<td>{confidence_chip(row.get("confidence"))}</td>
<td>{evidence_link(plate_url, "🏷️ Plate")}</td>
<td>{evidence_link(vehicle_url, "🚗 Extended")}</td>
</tr>"""
        rows_html.append(row_html)

    # Note: No leading spaces on the HTML string to avoid Streamlit Markdown parsing issues
    table_html = textwrap.dedent(f"""\
<div class="aegis-table-wrap">
<table class="aegis-table">
<thead>
...
if selected_room == "🏠 Executive Dashboard":
    ...
    with st.form("manual_detection_form"):
        ...
        with col_b:
            vehicle_type = st.selectbox("Vehicle Type", ["car", "bike", "bus", "truck"])
            vehicle_color = st.selectbox("Vehicle Color", ["unknown", "black", "white", "silver/gray", "red", "green", "blue", "yellow", "orange", "brown"])
        with col_c:
            confidence = st.slider("Confidence", 0.0, 1.0, 0.90, 0.01)
            evidence = st.text_input("Extended Crop URL", placeholder="/static/crops/MH03DY5705_vehicle.jpg")
